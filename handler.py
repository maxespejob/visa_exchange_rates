...
def split_into_chunks(items: list, num_chunks: int) -> list[list]:
    ...
    try:
        ...
        logger.info(
            f"[split_into_chunks] {len(items)} items split into {num_chunks} chunks (~{chunk_size} each)"
        )
        return chunks
        ...
        async def browser_worker(worker_id: int):
            ...
            while True:
                ...
                try:
                    ...
                    logger.info(
                        f"[scrape_chunk] worker={worker_id} | [{index}/{total}] OK {from_currency}->{to_currency} | fx={fx_rate}"
                    )

                except (json.JSONDecodeError, KeyError) as e:
                    logger.error(
                        f"[scrape_chunk] worker={worker_id} | [{index}/{total}] Failed to parse {from_currency}->{to_currency} | {e}"
                    )
                    results.append(
                        {
                            "date": date_str,
                    ...
                    logger.error(
                        f"[scrape_chunk] worker={worker_id} | [{index}/{total}] Unexpected error {from_currency}->{to_currency} | {e}"
                    )
                    results.append(
                        {
                            "date": date_str,
        ...
        for date in dates:
            ...
            for chunk_id, chunk in enumerate(chunks, start=1):
                try:
                    ...
                    logger.error(
                        f"[ORCHESTRATOR] Failed to invoke worker {chunk_id} for {date}: {e}"
                    )

        total_workers = NUM_CHUNKS * len(dates)
        logger.info(
            f"[ORCHESTRATOR] Done | {total_workers} workers launched | {len(pairs)} total pairs"
        )
